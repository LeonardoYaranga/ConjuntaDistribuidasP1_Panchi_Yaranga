...
public class EvaluadorRiesgoMedio extends EvaluadorRiesgo {
    @Override
    public boolean aplica(ec.edu.espe.examen_yaranga_leonardo.dto.ClienteDTO clienteDTO) {
    ...
    public EvaluacionDTO evaluar(ClienteDTO clienteDTO) {
        ...
        int puntaje = calcularPuntaje(clienteDTO);
        evaluacion.setNivelRiesgo("MEDIO");
        evaluacion.setAprobado(true);
        evaluacion.setPuntajeFinal(puntaje);
        evaluacion.setMensaje("Cliente apto para préstamo con condiciones ajustadas");
        evaluacion.setTasaInteres(8.0);
